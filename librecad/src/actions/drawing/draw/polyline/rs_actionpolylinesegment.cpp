...
void RS_ActionPolylineSegment::init(const int status) {
 ...
 * @author Rallaz
 */
RS_Vector RS_ActionPolylineSegment::appendPol(RS_Polyline* current, const RS_Polyline* toAdd, const bool reversed) const {
 ...
 * @author Rallaz
 */
void RS_ActionPolylineSegment::convertPolyline(RS_Entity* selectedEntity, const bool useSelected, LC_DocumentModificationBatch& ctx) const {
...
void RS_ActionPolylineSegment::doTriggerCompletion(const bool success) {
    if (success) {
        m_targetEntity = nullptr;
...
void RS_ActionPolylineSegment::onMouseMoveEvent([[maybe_unused]] const int status, const LC_MouseEvent* event) {
...
void RS_ActionPolylineSegment::onMouseLeftButtonRelease(const int status, const LC_MouseEvent* e) {
    switch (status) {
        case ChooseEntity:
...
void RS_ActionPolylineSegment::onMouseRightButtonRelease(const int status, [[maybe_unused]] const LC_MouseEvent* e) {
    deleteSnapper();
    if (m_targetEntity) {
        redraw();
    }
