 ...
 */
RS_ActionSelectIntersected::RS_ActionSelectIntersected(LC_ActionContext* actionContext, const bool select)
...
void RS_ActionSelectIntersected::init(const int status) {
    reset();
...
void RS_ActionSelectIntersected::selectionFinishedByKey([[maybe_unused]] QKeyEvent* e, [[maybe_unused]] bool escape) {
    finish();
}

void RS_ActionSelectIntersected::onMouseMoveEvent(const int status, const LC_MouseEvent* e) {
    if (status == SetPoint1) {
        if (e->isShift) {
            RS_Entity* entity;
            if (e->isControl) {
...
void RS_ActionSelectIntersected::onMouseLeftButtonPress(const int status, const LC_MouseEvent* e) {
...
void RS_ActionSelectIntersected::onMouseLeftButtonRelease(const int status, const LC_MouseEvent* e) {
...
bool RS_ActionSelectIntersected::isValidEntityForSelection(const RS_Entity* entity) {
...
void RS_ActionSelectIntersected::onMouseRightButtonRelease(const int status, [[maybe_unused]] const LC_MouseEvent* e) {
    RS_DEBUG->print("RS_ActionSelectIntersected::mouseReleaseEvent()");
...
void RS_ActionSelectIntersected::updateMouseButtonHints() {
    switch (getStatus()) {
        case SetPoint1:
            updateMouseWidgetTRCancel(tr("Choose first point of intersection line") + " " + getSelectionCompletionHintMsg(),
