...
namespace {
    class LC_SelectiveImportDialog : public QDialog {
    public:
        explicit LC_SelectiveImportDialog(QWidget* parent) : QDialog(parent) {
...
LC_ViewportThemeEditor::LC_ViewportThemeEditor(QObject* parent, QWidget* previewWidget)
    : QObject(parent), m_previewWidget(previewWidget) {
    m_repository.initializeIndex();
    m_activeKey = CFG_AppStyling::o_ActiveGraphicViewScheme.get();
}

LC_PresetManagerUIStrings LC_ViewportThemeEditor::presetStrings() const {
    ...
    s.presetFileFilter = tr("Drawing Visual Scheme Files (*.lsvs)"); // fixme - change!!!
    return s;
}

bool LC_ViewportThemeEditor::resetToDefaults(const QString& key) {
    initMappingTable();

    // 1. REGISTRY RESET: Force all settings in theme scope back to C++ defaults
    // This ensures uninitialized pages are also reset in the RS_Settings cache.
    ...
    // 2. UI RESET: Notify the dialog to refresh visible widgets
    if (m_resetCallback) {
        m_resetCallback();
    }

    if (m_changedCallback) m_changedCallback(false);
    return true;
}

bool LC_ViewportThemeEditor::loadPreset(const QString& key) {
    bool success = false;
    ...
    if (success) {
        m_activeKey = key;
        // Persist the active scheme key globally
...
bool LC_ViewportThemeEditor::saveCurrentPreset() {
    if (m_activeKey == DEFAULT_THEME_KEY) {
        return false; // Default theme is read-only template
    }
    LC_ViewportThemeConfig config;
...
bool LC_ViewportThemeEditor::savePresetAs(const QString& name, QString& outKey) {
    LC_ViewportThemeConfig config;
    config.name = name;
...
bool LC_ViewportThemeEditor::deletePreset(const QString& key) {
    if (key == DEFAULT_THEME_KEY) {
        return false;
    }
...
bool LC_ViewportThemeEditor::calculateDivergence() const {
    if (m_activeKey == DEFAULT_THEME_KEY || m_activeKey.isEmpty()) {
        return false;
    }

    initMappingTable();
    LC_ViewportThemeConfig config;
    // Disk I/O happens only here (Initial load / Palette switch)
    ...
    for (const auto& entry : g_viewportThemeMappingTable) {
        ...
        // Compare Registry value to JSON value
...
bool LC_ViewportThemeEditor::isPresetModified()  {
    // Simply return the pre-calculated flag. No logic side-effects here.
    return m_registryDiverged;
}

QList<QPair<QString, QString>> LC_ViewportThemeEditor::getAvailablePresets() const {
    auto choices = m_repository.getPresetChoices();
    choices.prepend(qMakePair(tr("Default Theme"), DEFAULT_THEME_KEY));
    return choices;
}

QString LC_ViewportThemeEditor::getActivePresetKey() const {
    return m_activeKey;
}

void LC_ViewportThemeEditor::applyCurrentPreset() {
    if (m_saveCommitCallback) {
        m_saveCommitCallback(); // Writes Widgets -> Registry
    }

    // Now the Registry differs from the JSON file
    m_registryDiverged = true;

    if (m_changedCallback) {
        m_changedCallback(false); // Widgets now match Registry, so m_isDirty = false
    }
}

void LC_ViewportThemeEditor::rollbackState() {
    loadPreset(m_activeKey);
}

void LC_ViewportThemeEditor::setChangedCallback(std::function<void(bool)> callback) {
    m_changedCallback = callback;
}

QJsonObject LC_ViewportThemeEditor::serializeCurrentSettings() const {
    QJsonObject root;
    initMappingTable();
    for (const auto& entry : g_viewportThemeMappingTable) {
        if (entry.component != LC_ViewportThemeComponent::Style)
        ...
        // val is already a Hex String for colors thanks to LC_Setting::getVariant()
        QVariant val = entry.setting->getVariant();

        // If the backend returned an invalid value, use the default
        if (!val.isValid() || (val.typeId() == QMetaType::QString && val.toString().isEmpty())) {
            val = entry.setting->defaultVariant();
            // Note: defaultVariant() also returns Hex for colors in your implementation
        }
...
void LC_ViewportThemeEditor::applyThemeJson(const QJsonObject& obj) {
    initMappingTable();
    for (const auto& entry : g_viewportThemeMappingTable) {
        if (entry.component != LC_ViewportThemeComponent::Style) {
            continue;
        ...
        if (val.isValid()) {
            // Write the value generically straight into the setting instance
...
bool LC_ViewportThemeEditor::importPresetFromFile(const QString& filePath, QWidget* parent) {
    QFile file(filePath);
    ...
    if (!m_repository.isCompatible(root)) {
        return false; // Verify layout signature compatibility
    }

    // Spawn our domain-specific selective import popup modal 
    LC_SelectiveImportDialog dlg(parent);
    if (dlg.exec() == QDialog::Accepted) {
        QSet<LC_ViewportThemeComponent> components;
        if (dlg.importStyle()) {
            components.insert(LC_ViewportThemeComponent::Style);
        }
        if (dlg.importBehavior()) {
            components.insert(LC_ViewportThemeComponent::Behavior);
        }
        if (dlg.importSizing()) {
            components.insert(LC_ViewportThemeComponent::Sizing);
        }

        // Apply strictly the selected components
        for (const auto& entry : g_viewportThemeMappingTable) {
            initMappingTable();
        ...
        RS_Settings::instance()->emitOptionsChanged(); // Force viewport repaint

        // Fire the changed callback so the host dialog reloads its active pages in-memory
        if (m_changedCallback) {
...
bool LC_ViewportThemeEditor::exportPresetToFile([[maybe_unused]]const QString& key, const QString& filePath,[[maybe_unused]] QWidget* parent) {    
    QFile file(filePath);
    if (!file.open(QIODevice::WriteOnly)) {
        return false;
    }

    // Serialize current options state
