...
protected:
    bool event(QEvent* e) override {
        ...
        if (e->type() == QEvent::WindowDeactivate || e->type() == QEvent::ApplicationDeactivated) {
...
LC_DetachedMenu::LC_DetachedMenu(const QString& title, const QList<QAction*>& actions, const LC_ProxyStyle* style, QMenu* sourceMenu,
                                 QWidget* parent)
    : QWidget(parent, Qt::Tool | Qt::FramelessWindowHint), m_sourceMenu(sourceMenu) {
...
void LC_DetachedMenu::populateMenu(QMenu* target, const QList<QAction*>& actions) {
...
void LC_DetachedMenu::resizeEvent(QResizeEvent* event) {
    ...
    if (m_titleBar) {
        ...
        m_titleBar->setGeometry(BORDER_WIDTH, BORDER_WIDTH, width() - 2 * BORDER_WIDTH, m_titleBar->sizeHint().height());
    }
}
