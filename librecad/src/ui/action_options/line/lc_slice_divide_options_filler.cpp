...
void LC_SliceDivideOptionsFiller::fillToolOptionsContainer(LC_PropertyContainer* container) {
    ...
    addBoolean({"a_relAngle", tr("Relative angle"), tr("If checked, tick angle is related to selected entity, otherwise it is absolute angle.")}, [action]()-> bool {
    ...
    addEnum({
                "a_tickType",
                tr("Tick Snap"),
                tr("Defines which point of tick should be placed to intersection point with selected entity")
    ...
    addLinearDistance({"a_tickLen", tr("Offset"), tr("Offset of tick snap point to intersection point")}, [action]() {
    ...
    addEnum({
                "a_edgeTick",
                tr("Edge Tick"),
                tr("Defines how ticks on entity edges should be placed. ")
            }, &edgeTickTypeDescriptor, [action]() -> LC_PropertyEnumValueType {
