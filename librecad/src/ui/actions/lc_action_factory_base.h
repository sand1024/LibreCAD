...
protected:
    struct ActionInfo{
        ...
        ActionInfo(const char* key, const QString& text, const char* iconName = nullptr, const QString& desc = QString())
            : key(key), actionType(RS2::ActionNone), text(text), iconName(iconName), description(desc) {}

        ActionInfo(const char* actionKey, const RS2::ActionType actionType, const QString& text,
                   const char* iconName = nullptr, const QString& desc = QString())
            : key(actionKey), actionType(actionType), text(text), iconName(iconName), description(desc) {}

        ActionInfo(const char* actionKey, void (QC_ApplicationWindow::*slotPtr)(), const QString& text,
                   const char* iconName = nullptr, const QString& desc = QString())
            : key(actionKey), actionType(RS2::ActionNone), text(text), iconName(iconName), slotPtr(slotPtr), description(desc) {}

        ActionInfo(const char* actionKey, void (QC_ApplicationWindow::*slotPtrBool)(bool), const QString& text,
                   const char* iconName = nullptr, const QString& desc = QString())
            : key(actionKey), actionType(RS2::ActionNone), text(text), iconName(iconName), slotPtrBool(slotPtrBool), description(desc) {}
    };
    ...
    struct ActionGroupInfo {
        ...
        QString description;
        const char* iconName{""};
        bool isShortcutConfigurable{true};
        bool isToolbarMenuConfigurable{true};
    };

    struct SpecialActionEntry {
    ...
    QAction* createSpecialAction(QMap<QString, QAction*>& map,
                                QActionGroup* group,
                                const char* key,
                                const QString& title,
                                const char* icon = nullptr,
    ...
    void createSpecialActions(QMap<QString, QAction*>& map,
                             QActionGroup* group,
    ...
    QAction* createAction_MW(const char* name, void (QC_ApplicationWindow::*slotPtr)(), void (QC_ApplicationWindow::*slotBoolPtr)(bool),
                            const QString& text, const char* iconName, QActionGroup* parent,
                            QMap<QString, QAction*>& actionsMap, bool useToggled = false,
                            const QString& description = QString()) const;
    ...
    QAction* justCreateAction(QMap<QString, QAction*>& actionsMap, const char* name, const QString& text, const char* iconName,
                              QActionGroup* parent, const QString& description = QString()) const;
    ...
    static void makeActionsExcludedFromRecent(const QMap<QString, QAction*>& map,
