    ...
    for (const LC_ActionGroup* group : groupsList) {
        if (group->isActionMappingsMayBeConfigured()) {
            ...
            for (const QAction* action : group->actions()) {
                ...
                // 2. Text Regex Match Evaluation
                bool hasRegexpMatch = false;
                if (!m_filterForConflicts && m_hasRegexp) {
                    ...
                    if (!m_regexpHighlightMode) {
                        if (!hasRegexpMatch) {
                            continue; // Skip: does not match text filter
                        }
                        hasRegexpMatch = false; // Reset in filter-only mode
                    }
                }

                // 3. Shortcut Key Sequence Match Evaluation (IntelliJ-style)
                ...
                if (!m_filterForConflicts && m_hasShortcutFilter) {
                    ...
                    if (!m_regexpHighlightMode) {
                        if (!hasShortcutMatch) {
                            continue; // Skip: does not match shortcut filter
                        }
                    }
                }

                // 4. Create Node and Set Match Flag (Highlights text in m_filteringColor)
                LC_ShortcutTreeItem* child = groupChild->addChild(action, shortcutInfo);
                const bool isMatched = (hasRegexpMatch || hasShortcutMatch);
...
bool LC_ShortcutsTreeModel::checkForCollisions(LC_ShortcutInfo* shortcutInfo) {
    ...
    if (shortcutInfo != nullptr) {
        // simple check, for this shortcut only
        ...
        // checking for all possible duplicates
        for (const auto currentShortcut : std::as_const(m_shortcuts)) {
...
void LC_ShortcutsTreeModel::applyShortcuts(const QMap<QString, QKeySequence>& map, const bool replace) {
    ...
    // Loading a preset establishes its shortcuts as the clean baseline
    if (replace) {
        commitBaseline();
    }
}
