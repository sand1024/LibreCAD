...
QString LC_QuickInfoEntityData::prepareGenericEntityDescription(const RS_Entity* e, const QString &entityTypeName, const RS2::EntityDescriptionLevel level){
    QString result;
    switch (level){
...
void LC_QuickInfoEntityData::collectCircleProperties(const RS_Circle *circle){
    ...
    addLinearProperty(tr("Circumference"), circumference, PROPERTY_TYPE_OTHER);
        ...
        switch (rtti) {
            case RS2::EntityArc: {
                ...
                if (m_options->displayPolylineDetailed){ // details of arc
                    ...
                    totalLengh += len;
                    addLinearProperty(tr("Circumference"), len, PROPERTY_TYPE_OTHER);
...
void LC_QuickInfoEntityData::addVectorProperty(const QString& name, const QString &valueStr, const RS_Vector &coord, const PropertyType type){
    auto *prop = new VectorPropertyInfo(name, valueStr, type, coord);
    m_properties << prop;
}

void LC_QuickInfoEntityData::addVectorProperty(QString name, const int count, const QString &valueStr, const RS_Vector &coord, const PropertyType type){
    QString idx;
    idx.setNum(count);
...
LC_QuickInfoEntityData::PropertyInfo::PropertyInfo(const QString &label, const QString &value, const int type):label(label), value(value), type(type){}

LC_QuickInfoEntityData::VectorPropertyInfo::VectorPropertyInfo(const QString &label, const QString &value, const int type, const RS_Vector &coord)
    :PropertyInfo(label, value, type),
     data(coord){}

LC_QuickInfoEntityData::DoublePropertyInfo::DoublePropertyInfo(const QString &label, const QString &value, const int type, const double d):PropertyInfo(label, value, type),
                                                                                                                               data(d){}
