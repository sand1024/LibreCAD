...
typedef QMap<QString, IndexEntry> PresetsIndexMap;

class LC_PresetsIndexHelper {
public:
    // Loads the index map from a given index filepath on disk
    static PresetsIndexMap loadIndex(const QString& indexFilePath);
    // Writes the index map back to disk
    static bool saveIndex(const QString& indexFilePath, const PresetsIndexMap& indexMap);
    ...
    static QStringList getOutOfDateFiles(const QString& directoryPath,
                                         ...
                                         const PresetsIndexMap& indexMap,
                                         PresetsIndexMap& outUpdatedIndex,
                                         bool& indexChanged);
};
