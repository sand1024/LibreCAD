...
bool RS_Ellipse::createFrom4P(const RS_VectorSolutions& sol) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        mt[i].resize(size + 1);
        const auto& solItem = sol.get(i);
        mt[i][0] = solItem.x * solItem.x;
        mt[i][1] = solItem.x;
        mt[i][2] = solItem.y * solItem.y;
        mt[i][3] = solItem.y;
        mt[i][4] = 1.;
    }
    ...
    if (std::abs(dn[0]) < RS_TOLERANCE15 || std::abs(dn[2]) < RS_TOLERANCE15 || d / dn[0] < RS_TOLERANCE15 || d / dn[2] < RS_TOLERANCE15) {
        //ellipse not defined
        return false;
    }
    m_data.center.set(-0.5 * dn[1] / dn[0], -0.5 * dn[3] / dn[2]); // center
    d = sqrt(d / dn[0]);
    m_data.majorP.set(d, 0.);
    m_data.ratio = sqrt(dn[0] / dn[2]);
    m_data.angle1 = 0.;
    m_data.angle2 = 0.;
    //    DEBUG_HEADER
...
void RS_Ellipse::rotate(const double angle) {
    ...
    m_data.center.rotate(aV);
    m_data.majorP.rotate(aV);
    calculateBorders();
}
...
void RS_Ellipse::setAngle1(const double a1) {
    m_data.angle1 = a1;
    m_data.isArc = std::isnormal(m_data.angle1) || std::isnormal(m_data.angle2);
}
