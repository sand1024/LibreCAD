...
private:
   ...
   */
  void appendLine(const RS_Line* line);
   ...
   */
  void appendSplinePoints(const LC_SplinePoints* spline);
   ...
   */
  void appendParabola(const LC_Parabola* parabola);
...
void PathBuilder::append(RS_Entity* entity) {
  ...
  const double tol = 1e-6; // fixme- sand - common tolerance constant
...
void PathBuilder::appendLine(const RS_Line* line) {
  if (!line) {
      return;
  }

  const QPointF uiEnd = toGuiPoint(line->getEndpoint());

  m_path.lineTo(uiEnd);
}

void PathBuilder::appendArc(const RS_Arc* arc) {
  ...
  const double startDeg = RS_Math::rad2deg(startAngle);
  const double sweepDeg = RS_Math::rad2deg(endAngle - startAngle);

  const QPointF center = toGuiPoint(arc->getCenter());
  const double radiusX = m_painter->toGuiDX(arc->getRadius());
  const double radiusY = m_painter->toGuiDY(arc->getRadius());
  const QPointF halfSize{radiusX, radiusY};
  const QRectF arcRect{center - halfSize, center + halfSize};

  m_path.arcTo(arcRect, startDeg, sweepDeg);
}

void PathBuilder::appendEllipse(const RS_Ellipse* ellipse) {
...
void PathBuilder::appendCircle(const RS_Circle* circle) {
  if (!circle || !m_painter) {
      return;
  }

  // TODO: need pixel level precision: issue #2035
  const QPointF center = toGuiPoint(circle->getCenter());
  const double radiusX = m_painter->toGuiDX(circle->getRadius());
  const double radiusY = m_painter->toGuiDY(circle->getRadius());
  const QPointF halfSize{radiusX, radiusY};
  const QRectF circleRect{center - halfSize, center + halfSize};

  m_path.addEllipse(circleRect);
}

void PathBuilder::appendSplinePoints(const LC_SplinePoints* spline) {
  if (!spline || !m_painter) {
      return;
  }
  ...
  for (size_t i = 0; i < num_segs; ++i) {
    const int seg_idx = static_cast<int>(i);
...
void PathBuilder::appendParabola(const LC_Parabola* parabola) {
  if (parabola == nullptr) {
      return;
  }
...
namespace LC_LoopUtils {
    ...
    bool LoopExtractor::validate() const {
        ...
        if (m_loop->count() == 1) {
            ...
            if (type == RS2::EntityEllipse) {
                const RS_Ellipse* ell = static_cast<RS_Ellipse*>(e);
            ...
            if (type == RS2::EntityParabola) {  // SUPPORT: Valid if primitives computed successfully
                const auto para = static_cast<LC_Parabola*>(e);
                return para->getParabolaData().valid;
            }
    ...
    RS_Entity* LoopExtractor::findFirst() const {
        ...
        const RS_Vector mid = firstEdge->getMiddlePoint();
        const RS_Vector lineStart(mid.x - 1000, mid.y);
        const RS_Line testLine(lineStart, mid);
        ...
        for (RS_Entity* e : m_data->unprocessed) {
            ...
            if (sol.hasValid()) {
                for (RS_Vector v : sol) {
                    const double d = v.distanceTo(lineStart);
                    if (d < minDist) {
                        minDist = d;
                        closest = e;
                    }
    ...
    RS_Entity* LoopExtractor::findOutermost(std::vector<RS_Entity*> edges) const {
        ...
        const RS_Circle circle(nullptr, RS_CircleData(m_data->endPoint, radius));
        const double currentAngle = m_data->current->getDirection2();  // Incoming tangent at junction
        std::vector<std::pair<double, RS_Entity*>> angleDiffs;
        for (RS_Entity* e : edges) {
            // Compute outgoing tangent, accounting for potential reversal
            const bool needsReversal = e->getStartpoint().distanceTo(m_data->endPoint) > ENDPOINT_TOLERANCE;
            ...
            for (size_t k = 0; k < sol.getNumber(); ++k) {
                ...
                const double dist = v.distanceTo(m_data->endPoint);
                if (std::abs(dist - radius) > RS_TOLERANCE) {
                    continue; // Filter valid radius intersections
                }

                const double a = (v - m_data->endPoint).angle();
                double adiff = RS_Math::correctAngle(a - outgoingAngle);
                if (adiff > M_PI) {
                    adiff = 2 * M_PI - adiff; // Minimal unsigned difference
                }
                if (adiff < min_adiff) {
                    min_adiff = adiff;
                    selected_v = v;
            ...
            if (min_adiff > 1e-4) {
                continue; // Skip if no close match (numerical or curvature issues)
            }

            const double angle = (selected_v - m_data->endPoint).angle();
            double diff = RS_Math::correctAngle(angle - currentAngle);
            if (diff > M_PI) {
                diff -= 2 * M_PI; // Signed difference in (-pi, pi]
            }
     ...
     */
    LoopSorter::LoopSorter(std::vector<std::unique_ptr<RS_EntityContainer>> loops) : m_data(std::make_unique<Data>()) {
     ...
     */
    LoopOptimizer::LoopOptimizer(const RS_EntityContainer& contour) : m_data{std::make_unique<Data>()}{
...
LC_Loops::LC_Loops(std::shared_ptr<RS_EntityContainer> loop, [[maybe_unused]] bool ownsEntities) : m_loop(loop) {
 ...
 */
QPainterPath LC_Loops::getPainterPath(RS_Painter* painter, const int level) const {
...
double LC_Loops::getTotalArea() const {
  return std::accumulate(m_children.begin(), m_children.end(), m_loop->areaLineIntegral(), [](const double area, const LC_Loops& loop) {
 ...
 */
RS_Vector LC_Loops::e_point(const RS_Vector& center, const double major, const double minor, const double rot, const double t) const {
 ...
 */
RS_Vector LC_Loops::e_prime(const double major, const double minor, const double rot, const double t) const {
 ...
 */
void LC_Loops::addEllipticArc(QPainterPath& path, const RS_Vector& center, const double major, const double minor, const double rot, const double a1, const double a2) const {
...
bool LC_Loops::isEntityClosed(const RS_Entity* e) const {
  ...
  if (type == RS2::EntityEllipse) {
      const auto ell = static_cast<const RS_Ellipse*>(e);
  ...
  if (type == RS2::EntitySpline) {  // SUPPORT: Closed splines
      const auto spl = static_cast<const LC_SplinePoints*>(e);
    return spl->isClosed();
  }
...
RS_Entity* LC_Loops::createSubEntity(RS_Entity* e, const RS_Vector& p1, const RS_Vector& p2) const {
  ...
  if (type == RS2::EntitySpline) {  // IMPROVED: Use cut() for precise trimming
  ...
  if (type == RS2::EntityParabola) {  // IMPROVED: Exact tangents via FromEndPointsTangents
      ...
      auto cps = para->getParabolaData().controlPoints;
...
std::vector<RS_Vector> LC_Loops::sortPointsAlongEntity(RS_Entity* e, std::vector<RS_Vector> inters) const {
  ...
  if (type == RS2::EntityLine) {
    ...
    auto para = static_cast<LC_Parabola*>(e);
    LC_ParabolaData& d = para->getParabolaData();
    if (!d.valid) {
        return {};
    }
  ...
  for (auto& [fst, snd] : param_points) {
      sorted.push_back(snd);
  }
  return sorted;
}
